...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`live_streams\` (
  ...
  `)
  await db.run(sql`CREATE INDEX \`live_streams_updated_at_idx\` ON \`live_streams\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`live_streams_created_at_idx\` ON \`live_streams\` (\`created_at\`);`)
  await db.run(sql`CREATE TABLE \`live_streams_locales\` (
  	\`title\` text NOT NULL,
  	\`description\` text,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`_locale\` text NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`live_streams\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`live_streams_locales_locale_parent_id_unique\` ON \`live_streams_locales\` (\`_locale\`,\`_parent_id\`);`)
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`live_streams_id\` integer REFERENCES live_streams(id);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.run(sql`DROP TABLE \`live_streams\`;`)
  await db.run(sql`DROP TABLE \`live_streams_locales\`;`)
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "users_id", "media_id", "videos_id", "spiritual_sessions_id", "coaching_services_id", "programs_id", "events_id", "faqs_id", "contact_messages_id", "resource_items_id", "blog_posts_id", "tips_id", "testimonials_id", "membership_plans_id", "courses_id", "program_stages_id", "enrollments_id", "subscriptions_id", "members_id") SELECT "id", "order", "parent_id", "path", "users_id", "media_id", "videos_id", "spiritual_sessions_id", "coaching_services_id", "programs_id", "events_id", "faqs_id", "contact_messages_id", "resource_items_id", "blog_posts_id", "tips_id", "testimonials_id", "membership_plans_id", "courses_id", "program_stages_id", "enrollments_id", "subscriptions_id", "members_id" FROM \`payload_locked_documents_rels\`;`)
